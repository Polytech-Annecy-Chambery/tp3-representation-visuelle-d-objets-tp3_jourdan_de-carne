...
def Q3a():
    return Configuration().add(
            Wall({'position': [1, 2, 0], 'width':7, 'height':2.6, 'orientation':40})
            ) 

def Q4a():
    # Ecriture en utilisant des variables : A compléter
    wall1 = Wall({'width':7, 'height':2.6})
    wall2 = Wall({'position': [0.2, -0.2, 0], 'width':4, 'height':2.6, 'orientation':90})
    wall3 = Wall({'position': [0.2, -7, 0], 'width':4, 'height':2.6, 'orientation':90})
    wall4 = Wall({'position': [0, 4.2, 0], 'width':7, 'height':2.6})  
...
def Q5c1():      
    section = Section({'position': [1, 2, 0],'width':7, 'height':2.6})
    opening1 = Opening({'position': [2, 0, 0], 'width':0.9, 'height':2.15, 'thickness':0.2, 'color': [0.7, 0.7, 0.7]})
    sections = section.createNewSections(opening1, [1,2,0])                                                                          #createOpening
...
def Q5c2():      
    section = Section({'width':7, 'height':2.6})
    opening2 = Opening({'position': [4, 0, 1.2], 'width':1.25, 'height':1, 'thickness':0.2, 'color': [0.7, 0.7, 0.7]}) 
    sections = section.createNewSections(opening2, [0,0,0])
    configuration = Configuration()
...
def Q5d():      
    wall = Wall({'position': [0, 0, 0], 'width':7, 'height':2.6, 'orientation':0})
    opening1 = Opening({'position': [2, 0, 0], 'width':0.9, 'height':2.15, 'thickness':0.2, 'color': [0.7, 0.7, 0.7]})
    opening2 = Opening({'position': [4, 0, 1.2], 'width':1.25, 'height':1, 'thickness':0.2, 'color': [0.7, 0.7, 0.7]}) 
    wall.add(opening2)
    wall.add(opening1)
